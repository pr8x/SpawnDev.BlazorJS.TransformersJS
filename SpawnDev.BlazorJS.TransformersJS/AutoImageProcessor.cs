...
namespace SpawnDev.BlazorJS.TransformersJS
{
    public class AutoImageProcessor : Pipeline
    {
        ...
        public VitPoseEstimatorOutput[][] PostProcessPoseEstimation(TensorProxy<float> heatmaps, (float, float, float, float)[][] boxes) => JSRef!.Call<VitPoseEstimatorOutput[][]>("post_process_pose_estimation", heatmaps, boxes);
        /// <summary>
        /// Transform the heatmaps into keypoint predictions and transform them back to the image.
        /// </summary>
        /// <param name="heatmaps"></param>
        /// <param name="boxes">List or array of bounding boxes for each image. Each box should be a list of 4 floats representing the bounding box coordinates in COCO format(top_left_x, top_left_y, width, height).</param>
        /// <returns>VitPoseEstimatorOutput</returns>
        public VitPoseEstimatorOutput[][] PostProcessPoseEstimation(TensorProxy<float> heatmaps, (int, int, int, int)[][] boxes) => JSRef!.Call<VitPoseEstimatorOutput[][]>("post_process_pose_estimation", heatmaps, boxes);
    }
}
